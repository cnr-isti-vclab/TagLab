...
#        self.undo = Undo()                       #not saved
    ...
    def selectAreas(self, target_w, target_h):

        pass

    def export_tiles(self, map, val_area, test_area, tile_size, step, basename, labels_info):
        """
        ...
        tile_cols = int((area_W - tile_size) / step)
        tile_rows = int((area_H - tile_size) / step)

        deltaW = (area_W - tile_size - tile_cols * step) / 2.0
        deltaH = (area_H - tile_size - tile_rows * step) / 2.0
        deltaW = int(deltaW)
        deltaH = int(deltaH)

        area_top = val_area[0] + tile_size/2
        area_left = val_area[1] + tile_size/2

        for row in range(tile_rows):
            for col in range(tile_cols):

                top = area_top + deltaH + row * step
                left = area_left + deltaW + col * step
                ...
                filenameRGB = os.path.join(basenameVim, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                           str.format("{0:02d}", (col)) + ".png")
                filenameLabel = os.path.join(basenameVlab, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                             str.format("{0:02d}", (col)) + ".png")
        ...
        for row in range(tile_rows):
            for col in range(tile_cols):
                ...
                filenameRGB = os.path.join(basenameTestIm, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                           str.format("{0:02d}", (col)) + ".png")
                filenameLabel = os.path.join(basenameTestLab, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                             str.format("{0:02d}", (col)) + ".png")
        ...
        area_top = tile_size/2
        area_left = tile_size/2
        ...
        for row in range(tile_rows):
            for col in range(tile_cols):
                ...
                bbox = [top, left, tile_size, tile_size]
                insideValArea = Split.isInsideBBox(bbox, val_area)
                insideTestArea = Split.isInsideBBox(bbox, test_area)
                if insideValArea is False and insideTestArea is False:

                    cropimg = utils.cropQImage(map, [top, left, tile_size, tile_size])
                    croplabel = utils.cropQImage(labelimg, [top, left, tile_size, tile_size])

                    filenameRGB = os.path.join(basenameTrainIm, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                           str.format("{0:02d}", (col)) + ".png")
                    filenameLabel = os.path.join(basenameTrainLab, "tile_" + str.format("{0:02d}", (row)) + "_" +
                                             str.format("{0:02d}", (col)) + ".png")
    ...
    def save_exported_areas(self, w, h, filename, labels_info, val_area, test_area):
        """
        Export a figure to show the validation and test area exported.
        """

        val_area[0] = int(val_area[0])
        val_area[1] = int(val_area[1])
        val_area[2] = int(val_area[2])
        val_area[3] = int(val_area[3])

        test_area[0] = int(test_area[0])
        test_area[1] = int(test_area[1])
        test_area[2] = int(test_area[2])
        test_area[3] = int(test_area[3])
        ...
        pen.setWidth(5)
        painter.setPen(pen)
        painter.drawRect(val_area[1], val_area[0], val_area[2], val_area[3])

        painter.setBrush(Qt.NoBrush)
        pen = QPen(Qt.blue)
        pen.setWidth(5)
        painter.setPen(pen)
        painter.drawRect(test_area[1], test_area[0], test_area[2], test_area[3])
