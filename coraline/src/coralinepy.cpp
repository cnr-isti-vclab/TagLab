...
//		Coraline* coraline = new Coraline(img, mask, w, h);
	CORALINE_EXPORT_C void Coraline_segment(uchar* img, uchar* mask, int w, int h, int *clippoints, int nclips, float lambda = 0.0, float conservative = 1.0, float grow = 0.0, float radius = 30) {
		Coraline* coraline = new Coraline(img, mask, w, h);
